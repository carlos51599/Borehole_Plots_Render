...
This module creates professional borehole logs with:
- Separate columns for lithology, depth, and layer description
- Color and hatch patterns from CSV geology code mapping
- Openground-style formatting and layout
- Professional typography and gridlines
"""
...
def plot_borehole_log_from_ags_content(
    ags_content,
    loca_id,
    show_labels=True,  # Not used but kept for compatibility
    fig_height=11.69,
    fig_width=4.0,
    ...
    """
    # Import parser from section_plot (shared logic)
...
class ProfessionalBoreholeLog:
    ...
    def __init__(self, geology_csv_path: Optional[str] = None):
        ...
        if geology_csv_path:
            try:
                self.geology_mapping = load_geology_mapping(geology_csv_path)
                logger.info(
    ...
    def create_borehole_log(
        ...
        figsize: Tuple[float, float] = (8.27, 11.69),  # A4 size
        dpi: int = 300,
    ) -> plt.Figure:
        """
        ...
        self._plot_lithology_column(ax_litho, borehole_data, min_depth, max_depth)
    ...
    def _plot_lithology_column(
        ...
        for _, row in data.iterrows():
            ...
            layer_props = self.geology_mapping.get(
