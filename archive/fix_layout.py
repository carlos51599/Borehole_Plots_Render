# Create layout.py with proper content
with open("borehole_log/layout.py", "w", encoding="utf-8") as f:
    content = '''# Professional Borehole Log Layout Module
    ...
    for i, (interval, legend_pos) in enumerate(zip(intervals, legend_positions)):
        ...
        if legend_pos["y_center"] > 0:  # Valid legend position
            ...
            if text_height <= layer_height:
                # Text fits within layer - extend text box to match layer boundary
                final_y_top = preferred_y_top
                final_y_bottom = layer_y_bottom  # Extend to layer boundary
                ...
                final_y_top = preferred_y_top
                final_y_bottom = preferred_y_top - text_height  # Extend to fit text
                extended_text_height = text_height

        else:
            # No valid legend position - handle gracefully
            final_y_top = 0
    ...
    while not conflicts_resolved and iteration < max_iterations:
        ...
        for i in range(1, len(text_positions)):
            ...
            if current["y_top"] > previous["y_bottom"]:
                ...
                # Check if the pushed-down text box is still within its layer boundaries
                # If so, extend it to match the layer bottom boundary
...
print("✅ Layout.py created successfully")
