...
try:
    ...
    logging.warning(f"Could not get network info: {str(e)}")
...
try:
    ...
    for pkg in pkg_resources.working_set:
        logging.info(f"  {pkg.project_name}=={pkg.version}")
except Exception as e:
    logging.warning(f"Could not list installed packages: {str(e)}")

# Check if dash_leaflet is installed and report its version
try:
    import dash_leaflet

    logging.info(f"dash_leaflet version: {dash_leaflet.__version__}")
except Exception as e:
    logging.error(f"Error importing dash_leaflet: {str(e)}")

try:
    app = dash.Dash(__name__, suppress_callback_exceptions=True)
    logging.info("Successfully created Dash app instance")
except Exception as e:
    logging.error(f"Error creating Dash app: {str(e)}", exc_info=True)
    raise
...
# Simple callback to test that callbacks are working
@app.callback(
    Output("server-info", "children"),
    Input("server-status", "n_clicks"),
)
def update_server_info(n_clicks):
    try:
        import platform
        import socket
