...
class GitAnalyzer:
    ...
    def _parse_change_frequency(self, git_output: str, days: int) -> Dict[str, Dict[str, Any]]:
    ...
    def _calculate_frequency_score(self, change_count: int, total_commits: int, days: int) -> float:
        ...
        frequency_score = (commit_percentage * 0.7) + (min(changes_per_day / 2, 1.0) * 0.3)
    ...
    def _parse_churn_analysis(self, git_output: str) -> Dict[str, Dict[str, Any]]:
        ...
        for file_path, stats in file_churn.items():
            if stats['commit_count'] > 0:
                stats['avg_churn_per_commit'] = stats['total_churn'] / stats['commit_count']
                stats['churn_classification'] = self._classify_churn(stats['total_churn'])
            else:
    ...
    def get_combined_analysis(self, days: int = 30) -> Dict[str, Any]:
        ...
        print(f"✅ Git analysis complete - found {len(combined_data)} files with changes")
    ...
    def _calculate_hotspot_score(self, frequency_data: Dict, churn_data: Dict) -> float:
        ...
        churn_score = min(churn_data.get('total_churn', 0) / 1000.0, 1.0)  # Normalize churn
